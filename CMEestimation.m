...
%% for tectonics
load ./pickleFiles/all_data_wo_seasn_trend_jump;
stns = stns';
...
%     close all;
    for dd=1:length(data)
        %For dN
        [vp, var_porc, eof_p_n, exp_coef_n] = eof_n_optimizado_A(data{dd});

        pcdata=exp_coef_n(:,mode);
        spatialdata=eof_p_n(:,mode);
        save(sprintf('MATLAB_output/pc%d_%s',mode, dtnm{dd}),'tdata','pcdata');
        Adata=[tdata;pcdata'];

        fileID = fopen(sprintf('MATLAB_output/pc%d_%s.txt',mode, dtnm{dd}),'w');
        ...
        writetable(T,sprintf('MATLAB_output/spatial%d_%s.txt',mode, dtnm{dd}),'Delimiter',' ')
...
%         m_pcolor(X,Y,(vq)); shading interp; colormap(flipud(colorMap));
        ...
        for i=1:length(slon)
            ...
            if length(idx) >= 1
                m_line(slon(i),slat(i),'marker','v','color',[0 0 0.5],'linest','none','markerfacecolor','r','markersize',10,'linewi',0.5);
            else
...
%         caxis([-1 1])
    %     h.Label.String = 'EOF ';
        ...
        print('-dpng','-r600',[sprintf('MATLAB_output/eof%d_CGPS_comp_spatial',mode), dtnm{dd}]);
        ...
        grid on;
        print('-dpng','-r600',[sprintf('MATLAB_output/eof%d_comp_temp_amp',mode), dtnm{dd}]);
